(function () {
    ...
    elements.forEach(element => {
      if (element.type === 'people') {
        members.forEach((person, index) => {
          ...
          person.y = currentY + row * CELL_H;
        });
        const rowsNeeded = Math.max(group.rows || 1, Math.ceil(members.length / group.cols));
        const heightUsed = rowsNeeded * CELL_H - (CELL_H - NODE_H);
  ...
  function elementBox(data, type, id) {
    ...
    if (type === 'person') {
      ...
      return person ? { x: person.x, y: person.y, w: NODE_W, h: NODE_H } : null;
    }
    if (type === 'label') {
  ...
  function renderGraph(container, data, options = {}) {
    ...
    const canvasHeight = Math.max(760, ...data.groups.map(g => g.y + g.h + 200), ...data.people.map(p => p.y + NODE_H + 200), ...data.labels.map(l => l.y + (Number(l.h) || LABEL_H) + 200));
    ...
    data.people.forEach(person => {
      ...
      el.innerHTML = `<div class="avatar" ${imageStyle}></div><strong class="person-name">${escapeHtml(person.name)}</strong><span class="person-group">${escapeHtml(person.traits || '')}</span>`;
      container.append(el);
